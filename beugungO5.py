...
def wavelength(g, d_1, d_proj=1, m=1):
    '''
    Alles in m eingeben
    ...
    print(f'Die Wellenlänge Lambda unter normalen Bedingungen beträgt {wavelength}± nm')
    
    return wavelength_kn

def gitterkonstant(d_1, d_proj, wave_lambda=532, m=1):
    '''
...
prompt_wavelength = input('Are you solving Aufgabe 4? Enter to skip\n')
...
if prompt_g:
    try:
        ...
        print('So..something went wrong')
        print(ne)
    prompt_use = input('Do you want to use your Lambda from the previous calculation? Enter to skip\n')
    if prompt_use:
        gitterkonstant(d_1,d_proj, wave_lambda=wave_lambda)
    else:
        gitterkonstant(d_1,d_proj)

#todo 
#verknupfe experimental lambda und unsicherheit zu
#how do i recall unsicherheit from 2 returns? 
